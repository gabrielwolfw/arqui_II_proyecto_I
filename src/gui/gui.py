...
class ProcessRunnerGUI(tk.Tk):
    ...
    def _make_widgets(self):
        ...
        # Por defecto usar pe_tests
        self.default_cmd = "./../bin/pe_tests"
        self.cmd_var.set(self.default_cmd)
        ...
        # Botón de stepping
        self.stepping_var = tk.BooleanVar(value=False)
        self.stepping_btn = ttk.Checkbutton(btn_frame, text="Modo Stepping", 
                                          variable=self.stepping_var,
                                          command=self.toggle_stepping)
        self.stepping_btn.pack(side="left", padx=4)
        
        # Observar cambios en stepping_var
        self.stepping_var.trace_add("write", self.update_command)

        # Botón de step
        self.step_btn = ttk.Button(btn_frame, text="Step", command=self.do_step, state="disabled")
    ...
    def update_command(self, *args):
        """Actualiza el comando cuando cambia el modo stepping"""
        current_cmd = self.cmd_var.get().strip()
        if self.stepping_var.get():
            # Si el comando actual es el default o termina en pe_tests, agregar -s
            if current_cmd == self.default_cmd or current_cmd.endswith("pe_tests"):
                self.cmd_var.set(f"{self.default_cmd} -s")
            elif "pe_tests" in current_cmd and "-s" not in current_cmd:
                self.cmd_var.set(f"{current_cmd} -s")
        else:
            # Quitar el flag -s si existe
            if current_cmd.endswith(" -s"):
                self.cmd_var.set(current_cmd[:-3])
            elif " -s " in current_cmd:
                self.cmd_var.set(current_cmd.replace(" -s ", " "))

    def toggle_stepping(self):
        """Maneja el cambio en el modo stepping"""
        if self.stepping_var.get():
    ...
    def do_step(self):
        """Envía un Enter al proceso para avanzar un paso"""
        if self.process and self.process.stdin:
            try:
                self.process.stdin.write("\n")
                self.process.stdin.flush()
            except Exception as e:
                self.append_meta(f"Error al enviar step: {e}\n")
    ...
    def start_process(self):
        ...
        # El comando ya incluye -s si el stepping está activado gracias al update_command
        ...
        self.stepping_btn.config(state="disabled")  # Deshabilitar cambio de modo stepping durante la ejecución
        if self.stepping_var.get():
            self.step_btn.config(state="normal")  # Habilitar step si el modo stepping está activo
        ...
        try:
            while True:
                ...
                if tag == "__proc_end__":
                    ...
                    self.stepping_btn.config(state="normal")  # Re-habilitar el control de stepping
                    self.step_btn.config(state="disabled")  # Deshabilitar step cuando termina el proceso
                    continue
